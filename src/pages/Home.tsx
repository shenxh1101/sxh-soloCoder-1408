...
import { Sparkles, ChevronLeft, ChevronRight, AlertCircle, Eye, RotateCcw } from 'lucide-react';
...
export default function Home() {
  ...
  const { sessionId, error, loading, viewingSnapshot, snapshotStepIndex, exitSnapshotView } = useDataStore();
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-slate-950 text-slate-100 overflow-hidden">
      ...
      {viewingSnapshot && (
        <div className="flex items-center justify-between gap-2 px-4 py-2 bg-blue-500/10 border-b border-blue-500/30 text-xs text-blue-300">
          <div className="flex items-center gap-2">
            <Eye className="w-3.5 h-3.5" />
            <span>正在查看第 <b className="text-blue-200">{snapshotStepIndex}</b> 步的数据快照（只读）</span>
          </div>
          <button
            onClick={exitSnapshotView}
            className="flex items-center gap-1 px-2 py-1 rounded bg-blue-600 hover:bg-blue-500 text-white text-[11px] transition"
          >
